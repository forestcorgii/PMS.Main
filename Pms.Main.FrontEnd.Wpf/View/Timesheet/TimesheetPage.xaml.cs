...
namespace Pms.Main.FrontEnd.Wpf
{
    ...
    public partial class TimesheetPage
    {
        ...
        private FilterTimesheetsViewModel FilterTimesheetsVM;
        private ExportTimesheetsViewModel ExportTimesheetsVM;
        private DownloadTimesheetsViewModel DownloadTimesheetsVM;
        private EvaluateTimesheetsViewModel EvaluateTimesheetsVM;


        private FilterEmployeesViewModel EmployeeController;
        private DownloadEmployeesViewModel EEDownloadController;
        ...
        private void GenerateDBFPage_Loaded(object sender, RoutedEventArgs e)
        {
            if (Shared.DefaultCutoff is not null && Shared.DefaultPayrollCode is not null)
            {
                ...
                EEDownloadController = new( Shared.DefaultPayrollCode);

                FilterTimesheetsVM = new(Shared.DefaultCutoff, Shared.DefaultPayrollCode);
                DownloadTimesheetsVM = new(Shared.DefaultCutoff, Shared.DefaultPayrollCode);
                ExportTimesheetsVM = new(Shared.DefaultCutoff, Shared.DefaultPayrollCode);
                EvaluateTimesheetsVM = new(Shared.DefaultCutoff, Shared.DefaultPayrollCode);
        ...
        private void ReloadList()
        {
            if (Shared.DefaultCutoff is not null && Shared.DefaultPayrollCode is not null)
            {
                IEnumerable<Timesheet> timesheets = FilterTimesheetsVM.GetTimesheets();
        ...
        {
            DownloadTimesheetsVM.DownloadStarted += DownloadController_DownloadStarted;
            DownloadTimesheetsVM.DownloadCancelled += DownloadController_DownloadEnded;
            DownloadTimesheetsVM.DownloadEnded += DownloadController_DownloadEnded;

            DownloadTimesheetsVM.PageDownloadSucceeded += DownloadController_PageDownloadSucceeded;
            DownloadTimesheetsVM.PageDownloadFailed += DownloadController_PageDownloadFailed;

            EvaluateTimesheetsVM.EvaluationStarted += DownloadController_EvaluationStarted;
            EvaluateTimesheetsVM.EvaluationSucceeded += DownloadController_EvaluationSucceeded;
            EvaluateTimesheetsVM.EvaluationFailed += DownloadController_EvaluationFailed;
            ...
            ExportTimesheetsVM.ExportStarted += TimesheetOutputController_ExportStarted;
            ExportTimesheetsVM.ExportFailed += TimesheetOutputController_ExportFailed;
            ExportTimesheetsVM.ExportEnded += TimesheetOutputController_ExportEnded;

            FilterTimesheetsVM.TimesheetFillStarted += TimesheetController_TimesheetFillStarted;
            FilterTimesheetsVM.TimesheetFilled += TimesheetController_TimesheetFilled;
            FilterTimesheetsVM.TimesheetFillFailed += TimesheetController_TimesheetFillFailed;
        }
